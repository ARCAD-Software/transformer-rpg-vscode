...
export async function convertMultipleMembers(data: CommandParams, memberToConvert: SourceMember): Promise<void> {
    const sourceMembersList: SourceMember[] = (await getMembersListWithProgress(memberToConvert))
        .map(member => ({ library: member.library, file: member.file, name: member.name, extension: member.extension }));
    if (sourceMembersList.length) {
        ...
        if (isConfirmtoConvert) {
            await lookupMemberObjectTypes(sourceMembersList, memberToConvert.library);
            await convertTargets(data, sourceMembersList, memberToConvert.library);
        }
    }
    else {
...
export async function getMembersListWithProgress(target: SourceMember): Promise<IBMiMember[]> {
...
export async function lookupMemberObjectTypes(targets: SourceMember[], library: string) {
    await window.withProgress({
        ...
        for (const target of targets) {
            ...
            if (target.name) {
                target.objectType = await findObjectType(library, target.name, cache);
