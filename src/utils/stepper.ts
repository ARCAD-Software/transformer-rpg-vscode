...
export class MultiStepInput {
    ...
    private async stepThrough<T>(start: InputStep) {
        ...
        while (step) {
            if (this.steps.length === 0 || this.steps[this.steps.length - 1] !== step) {
    ...
    public getCurrentQuickPick(): QuickInput | undefined {
        return this.current;
    }

    public setBusy(busy: boolean) {
        if (this.current) {
            this.current.busy = busy;
            this.current.enabled = !busy;
        }
    }
}